...
export default function AvatarUpload({ uid, onUploaded }: AvatarUploadProps) {
  ...
  return (
    <div className="grid gap-2">
      <label className="text-xs text-[color:var(--muted)]">Avatar upload</label>
      <input
        type="file"
        accept="image/*"
        onChange={(event) => {
      ...
      />
      {uploading ? (
        <div className="text-xs text-[color:var(--muted)]">Uploading...</div>
      ) : null}
      {error ? (
        <div className="text-xs text-[color:var(--accent)]">{error}</div>
      ) : null}
    </div>
  );
}
