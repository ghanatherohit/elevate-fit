  ...
  return (
    <motion.div className="grid gap-5" variants={container} initial="hidden" animate="show">
      ...
      <motion.div variants={item}>
        <GlassCard className="grid gap-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-xs text-[color:var(--muted)]">Workout day key</div>
              <div className="text-sm font-semibold text-[color:var(--text)]">Weekly split overview</div>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {workoutDayKey.map((item) => (
              <div
                key={item.day}
                className="rounded-full border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-1 text-[11px] text-[color:var(--text)]"
              >
          ...
          <div className="flex flex-wrap gap-2">
            {allSections.map((section) => (
              <button
                ...
                className="rounded-full border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-1 text-[11px] text-[color:var(--muted)] transition hover:border-[color:var(--accent)] hover:text-[color:var(--text)]"
              >
                {section.shortTitle}
              </button>
            ))}
      ...
      <motion.div variants={item}>
        <GlassCard className="grid gap-4">
          <div>
            <div className="text-xs text-[color:var(--muted)]">Select a muscle</div>
            <div className="text-sm font-semibold text-[color:var(--text)]">Body map</div>
            <div className="text-[11px] text-[color:var(--muted)]">Click the body to reveal a muscle label. Rotate to see the back.</div>
          </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-[color:var(--border)] bg-[radial-gradient(circle_at_top,_rgba(124,108,255,0.12),_rgba(16,21,33,0.95)_60%)]">
            <BodyMap3D
              view="front"
        ...
        <div className="grid gap-4">
          {dayPlans.map((plan, index) => (
            <details
              ...
              className="rounded-3xl border border-[color:var(--border)] bg-[color:var(--card)]"
              open={index === 0}
            >
              <summary className="cursor-pointer list-none px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <div className="text-xs text-[color:var(--muted)]">Workout day</div>
                    <div className="text-sm font-semibold text-[color:var(--text)]">{plan.title}</div>
                  </div>
                  <div className="rounded-full border border-[color:var(--border)] px-3 py-1 text-[10px] text-[color:var(--muted)]">
              ...
              <div className="grid gap-4 border-t border-[color:var(--border)] px-4 py-4">
                {plan.subtitle ? <div className="text-xs text-[color:var(--muted)]">{plan.subtitle}</div> : null}
                {plan.blocks.map((block) => (
                  <div key={block.id} className="grid gap-2">
                    <div className="text-sm font-semibold text-[color:var(--text)]">{block.title}</div>
                    {block.note ? <div className="text-xs text-[color:var(--muted)]">{block.note}</div> : null}
                    <div className="grid gap-2">
                      <div
                        className="grid gap-2 text-[10px] uppercase tracking-wide text-[color:var(--muted)]"
                      ...
                      {block.rows.map((row, rowIndex) => (
                        <div
                          ...
                          className="grid gap-2 border-t border-[color:var(--border)] pt-2 text-[11px] text-[color:var(--text)]"
                          style={{ gridTemplateColumns: getGridTemplate(block.columns.length) }}
                        >
          ...
          {atlasSections.map((section) => (
            ...
            >
              <summary className="cursor-pointer list-none px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <div className="text-xs text-[color:var(--muted)]">Muscle atlas</div>
                    <div className="text-sm font-semibold text-[color:var(--text)]">{section.title}</div>
                  </div>
                  <div className="rounded-full border border-[color:var(--border)] px-3 py-1 text-[10px] text-[color:var(--muted)]">
              ...
              <div className="grid gap-4 border-t border-[color:var(--border)] px-4 py-4">
                {section.subtitle ? <div className="text-xs text-[color:var(--muted)]">{section.subtitle}</div> : null}
                {section.note ? <div className="text-xs text-[color:var(--muted)]">{section.note}</div> : null}
                {section.bullets ? (
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {section.images.map((image) => (
                      <img
                        ...
                        src={image.src}
                        alt={image.alt}
                        className="h-[220px] w-full rounded-2xl border border-[color:var(--border)] object-cover"
                        loading="lazy"
                      />
                    ))}
                  </div>
