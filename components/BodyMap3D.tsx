...
const BodyModelGLTF = ({
  ...
  modelUrl: string;
  children: (data: { model: Object3D; size: Vector3; center: Vector3 }) => JSX.Element;
}) => {
  ...
  const ModelWithHotspots = ({
    ...
    return (
      <group
        ...
        onPointerMove={(event) => updateHover(event.point)}
      ...
      >
        ...
        {selectedSpot ? (
          <Html center position={selectedSpot.localPosition} distanceFactor={5} transform>
            <div className="pointer-events-none rounded-full border border-[color:var(--border)] bg-[color:var(--card-strong)] px-2 py-0.5 text-[9px] text-[color:var(--text)] shadow-sm">
              {selectedSpot.label}
            </div>
          </Html>
        ) : null}
        ...
        {debugPoint && debugNormalized ? (
          <Html position={[bounds.min.x, bounds.min.y, bounds.min.z]} distanceFactor={6} transform>
            <div className="pointer-events-none rounded-xl border border-[color:var(--border)] bg-[color:var(--card-strong)] px-2 py-1 text-[9px] text-[color:var(--muted)] shadow-sm">
    ...
    <div className="h-[420px] w-full">
      <Canvas camera={{ position: cameraPosition, fov: 30 }} dpr={[1, 1.5]}>
        ...
        <Suspense
          fallback={
            <Html center>
              <div className="rounded-full border border-[color:var(--border)] bg-[color:var(--card-strong)] px-3 py-1 text-[11px] text-[color:var(--muted)]">
                Loading model
              </div>
            </Html>
          }
